...
class MessageRepository:
    ...
    async def save_messages_batch(
        ...
        async with self.session.begin():
            stmt = (
                insert(Message)
                .values([self.prepare_message_obj(msg) for msg in messages])
        ...
        async with self.session.begin():
            stmt = (
                ...
                .on_conflict_do_update(
                    ...
                    set_=self.omit_key_fields(
                        message_obj, ["message_id", "chat_id"]
                    ),
                )
            )
            await self.session.execute(stmt)
    ...
    async def get_first_message_id(self, chat_id: int) -> int:
        async with self.session.begin():
            result = await self.session.execute(
                select(func.min(Message.message_id)).where(
                    Message.chat_id == chat_id
                )
            )
